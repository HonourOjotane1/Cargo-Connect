...
def create_order(
    ...
    estimated_price = calculate_estimated_price(
        delivery_type=order_data.delivery_type.value, distance=distance, weight=weight
    )
    # This stores this price in the order or returns it in the response.
    order = Order(**order_data.dict(), user_id=user_id, estimated_price=estimated_price)
    db.add(order)
    db.commit()
    db.refresh(order)
